"use strict";
// This is a version of babel-plugin-minify-dead-code-elimination/src/index.js
// with https://github.com/babel/babili/commit/48635c464aa6e40b2a3befdc50a83b2f54fda42e
// reverted.
//
const { markEvalScopes, isMarked: isEvalScopesMarked, hasEval } = require("./mark-eval-scope.js");

function some(collection, pred) {
  // replaces require("lodash.some")
  if (Array.isArray(collection)) {
    return collection.some(pred)
  }
  return Object.keys(collection).some(k => pred(collection[k]))
}
...
module.exports = ({ types: t, traverse }) => {
  ...
  const main = {
    ...
    ConditionalExpression(path) {
      const { node } = path;
      // see packages/babel-traverse/src/path/evaluation.js
  ...
  return {
    ...
    visitor: {
      ...
      Program(path, {
        // exit(path, { // 48635c464aa6e40b2a3befdc50a83b2f54fda42e
          opts: {
            // set defaults
          ...
          if (cfg.verbose > 1) {
            console.time(`[dce] ${this.file.opts.sourceFileName}`)
          }
          ...
          path.traverse(main, {
            ...
            optimizeRawSize: true, // hoist all vars to function top
            keepFnName: !cfg.compress,
            keepClassName: !cfg.compress,
            keepFnArgs,
          });

          if (cfg.verbose > 1) {
            console.timeEnd(`[dce] ${this.file.opts.sourceFileName}`)
          }
        // } // 48635c464aa6e40b2a3befdc50a83b2f54fda42e
      },
    },
  };
