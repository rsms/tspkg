    ...
    VariableDeclarator(path) {
      ...
      if (hasRelativePathPrefix(source.value)) {
        ...
        if (path.scope.parent) {
          // not at root level -- leave it untouched and note that we need `require`
        ...
        if (existingId) {
          ...
          // if (this.file.opts.sourceFileName == 'a.ts') {
          //   let b = path.scope.getOwnBinding(id.node.name)
          //   console.log('reg external import', id.node.name, source.value)
          //   dump(path.node, 1)
          //   // if (src.id == 'assert') {
          //     console.log('binding:')
          //     dump(path.scope.bindings, 1)
          //   // }
          // }
          if (id.node.loc) {
    ...
    visitor: {
      ...
      Program: {
        ...
        exit(path) {
          ...
          // console.log('path.scope.bindings 1:', Object.keys(path.scope.bindings))
