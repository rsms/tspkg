...
function resolveModulePath(importerPath, importPath, fileMap, isRelative) {
  ...
  return fileMap.has(path)                       ? path :
         ...
         fileMap.has(path2 = path + '/index.ts') ? path2 :
         fileMap.has(path2 = path + '/index.js') ? path2 :
                                                   null
}
...
function resolveModuleId(importPath, sourceFiles, cfg, file) {
  ...
  const isRelative = hasRelativePathPrefix(importPath)
  let srcpath = resolveModulePath(selfFilename, importPath, sourceFiles, isRelative)

  if (!srcpath) {
    if (!isRelative) {
      // external module
      const filename = stripFileExt(importPath, stripExternalFileExts)
      return {
        id: moduleIdFromFilename(filename),
        internal: false,
        filename,
      }
    } else {
      throw TSPkgError(null, TSPkgError.UNRESOLVED_DEPENDENCY,
        `Unresolved dependency "${importPath}" imported by "${selfFilename}"`)
    }
  }
...
module.exports = exports.default = function ({types: t}) {
  ...
  const pkgVisitor = {
    ...
    CallExpression(path, state) {
      ...
      if (isValidRequireCall(path)) {
        ...
        if (src.internal) {
          ...
          this.usesRequireFunc = true // note that we need `require`
    ...
    VariableDeclarator(path) {
      ...
      if (src.internal) {
        ...
        // external dependency
        if (path.scope.parent || !path.node.init._convertedFromImport) {
          // Not at root level or not converted from import (i.e. is actual call to `require`).
          // Leave it untouched and note that we need `require`
          this.usesRequireFunc = true
          return
        }

        // is converted from import
  ...
  return {
    ...
    pre(file) {
      ...
      this.exportsId = '_$' + this.moduleId
    ...
    visitor: {
      ...
      Program: {
        ...
        exit(path) {
          ...
          const cfg = this.opts.tspkgConfig
          if (cfg.verbose > 1) {
            console.time(`[transform-pkg] ${this.file.opts.sourceFileName}`)
          }

          const { node, scope } = path;

          const moduleId = this.moduleId
          const exportsId = this.exportsId
          const localIdPrefix = '_$' + moduleId + '$'

          // drop any directives e.g. "use strict"
          node.directives = []

          path.traverse(pkgVisitor, this)
          ...
          function getBinding(name) {
            return path.scope.getOwnBinding(renamedBindings.get(name) || name)
          }
          ...
          if (this.moduleExport.value) {
            ...
            for (let [expName, exp] of this.exports) {
              ...
              if (t.isExpressionStatement(exp.path.parentPath)) {
                ...
                exportsProps.push(
                  t.objectProperty(t.identifier(expName), b.identifier)
                )
          ...
          for (let p of path.get('body')) {
                ...
                expr.left.object.name == exportsId )
            {
              if (rightIsNull(expr.right)) {
                ...
                console.warn("[transform-pkg] TODO: can't safely eliminate dead path")
          ...
          function importIsUsed(imp) {
            for (let s of imp.specifiers) {
              if (getBinding(s.local)) {
                // import spec IS being used
                return true
              }
            }
            return false
          }
          const imps = this.file.metadata.modules.imports
          for (let i = 0; i != imps.length; ++i) {
            const imp = imps[i]
            if (!importIsUsed(imp)) {
              imps.splice(i, 1)
              --i
              this.externalImports.delete(imp.source)
            }
          }

          // simplify externalImports to {source => id}
          ...
          if (cfg.verbose > 1) {
            console.timeEnd(`[transform-pkg] ${this.file.opts.sourceFileName}`)
          }
          ...
          this.file.metadata.tspkg = {
            ...
            imports: {
              ...
              usesRequireFunc: this.usesRequireFunc,
            },
          }
        }
      }
