...
module.exports = exports.default = function ({types: t}) {
    ...
    CallExpression(path, state) {
      ...
      if (isValidRequireCall(path)) {
        ...
        if (hasRelativePathPrefix(source.value)) {
          // internal dependency
          path.replaceWith(t.identifier('_$' + sourceModuleId))
        } else {
          // external dependency
          let id = this.externalImports.get(source.value)
          if (!id) {
            id = path.scope.generateUidIdentifier()
            id._externalModuleId = sourceModuleId
  ...
  return {
    ...
    pre(file) {
      ...
      this.externalImportExprs = new Map // source => path
    ...
    visitor: {
      ...
      Program: {
        ...
        exit(path) {
          ...
          const externalImports2 = new Map
          this.externalImports.forEach((id, source) => {
            externalImports2.set(source, id.name)
          })
