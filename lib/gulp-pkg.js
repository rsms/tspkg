...
module.exports = function(cfg) {
  ...
  function finalize(cb) {
    ...
    // true if we need "require"
    let usesRequireFunc = false
    ...
    for (let [filename, file] of inFiles) {
      ...
      if (!usesRequireFunc) {
        usesRequireFunc = file.babel.tspkg.imports.usesRequireFunc
      }
    ...
    const requireRef   = usesRequireFunc ? 'R,'         : ''
    const requireArg   = usesRequireFunc ? '"require",' : ''
    const requireParam = usesRequireFunc ? 'require,'   : ''
    const sliceStart   = usesRequireFunc ? 2 : 1
    ...
    ( typeof define != "undefined" ? define :
      (function(g,R) { return function(i,d,f){
        ...
        f.apply(g,
          ...
          ].concat(d.slice(${sliceStart}).map(R))
        );
      };})(
        this,
    ...
    [${requireArg}"exports"${importRefsJS}],
    function(${requireParam}E${importIdsJS}) {
